  ...
  if (result.status === 'manual') {
    if (result.error) {
      ...
      if (result.error.response?.data) {
        ...
        console.error(
          `An unknown error occurred while trying to create a release on GitHub, please report this on https://github.com/Jordan-Hall/nxrc (NOTE: make sure to redact your GitHub token from the error message!)`
        );
      }
    }
